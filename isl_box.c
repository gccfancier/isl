 ...
 */
static __isl_give isl_fixed_box *isl_fixed_box_set(
 ...
 */
static __isl_give isl_fixed_box *isl_fixed_box_set_valid(
	__isl_take isl_fixed_box *box, int pos, __isl_keep isl_aff *offset,
	__isl_keep isl_val *size)
{
	isl_bool valid;
	...
	if (valid < 0 || !valid)
		return box;
	return isl_fixed_box_set(box, pos, offset, size);
}
	...
	__isl_take isl_fixed_box *box)
{
	...
	for (i = 0; i < n; ++i)
		box = isl_fixed_box_set(box, i, nan, infty);
	isl_aff_free(nan);
	isl_val_free(infty);
	...
	__isl_keep isl_map *map, int pos)
{
	...
	if (valid < 0)
		...
		box = isl_fixed_box_set_valid(box, pos, info.offset, info.size);
	else
